...
app_script = app_script+'			{ zwave_id = S(msg).between(\': Z\', \'_ON\').s; }\n';
...
app_script = app_script+'			{ zwave_id = S(msg).between(\'<rf>ZWAVE Z\',\'</rf>\').s; }\n';
app_script = app_script+'			//console.log("Debug (1) : zwave     : " + zwave + " | Composant " + zwave_id + " | Statut : " + zwave_status);\n';
app_script = app_script+'		}\n';
app_script = app_script+'		//Thu Aug 14 2014 23:04:47 GMT+0100 (BST) Sent radio ID (1 Burst(s), Protocols=\'ZWave n38\'  Last RF Transmit Time=10ms): ZC7_OFF\n';
...
app_script = app_script+'			{ zwave_id = S(msg).between(\': Z\', \'_OFF\').s; }\n';
app_script = app_script+'			else if ((S(msg).include("ZWAVE")))	//Received radio ID (<rf>ZWAVE ZC4</rf> <dev>WakeUp</dev> Batt=<bat>Ok</bat>): <id>WZC4</id>\n';
app_script = app_script+'			{ zwave_id = S(msg).between(\'<rf>ZWAVE Z\',\'</rf>\').s; }\n';
app_script = app_script+'			//console.log("Debug (2) : zwave     : " + zwave + " | Composant " + zwave_id + " | Statut : " + zwave_status);\n';
app_script = app_script+'		}\n';
...
app_script = app_script+'			{ zwave_id = S(msg).between(\'<rf>ZWAVE Z\',\'</rf>\').s; }\n';
app_script = app_script+'			//console.log("Debug (3) : zwave     : " + zwave + " | Composant " + zwave_id + " | Statut : " + zwave_status);\n';
app_script = app_script+'		}\n';
app_script = app_script+'		console.log("Debug Detection ZWAVE : zwave     : " + zwave + " | Composant " + zwave_id + " | Statut : " + zwave_status);\n';
app_script = app_script+'	}\n';
...
request(zibase_url, function(err, resp, body)
{
	...
	parseString(body, function(err, result)
	{
		result.r.e.forEach (function(i)
		{
			...
			{ console.log("Lignes : " + count + ", nom : " + name_eqp + ", Protocole " + proto_i +  ", id : " + id1_eqp+ " / " + id2_eqp+ ", type_eqp : " + type_eqp); }
			//console.log( "nom: " + i.n + ", id: " + i['$'].c + ", type_eqp: " + i['$'].t)
			else console.log("Lignes : " + count + ", nom : " + name_eqp + ", Protocole " + proto_i +  ", id : " + id_eqp+ ", type_eqp : " + type_eqp);
